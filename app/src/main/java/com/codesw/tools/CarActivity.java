	...
	private void initialize(Bundle _savedInstanceState) {
		...
		materialbutton1.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				car = Double.parseDouble(edittext1.getText().toString());
				r = (Double.parseDouble(edittext2.getText().toString()) / 100) / 12;
				n = Double.parseDouble(edittext3.getText().toString());
				payment = (car * r) / (1 - Math.pow(1 + r, 0 - n));
				d.setTitle("Loan Calculation");
				d.setMessage("Monthly payment\n  $".concat(String.valueOf(payment)).concat("\nTotal Payment \n   $".concat(String.valueOf(car + ((payment * n) - car))).concat("\nTotal Interest\n   $".concat(String.valueOf((payment * n) - car)))));
